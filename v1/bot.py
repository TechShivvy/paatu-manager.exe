...
def create_spotify(username=None):
    auth_manager = None
    if username:
...
def refresh_spotify(auth_manager, spotify, username=None):
    token_info = auth_manager.cache_handler.get_cached_token()
    if auth_manager.is_token_expired(token_info):
        auth_manager, spotify = create_spotify(username)
    return auth_manager, spotify
...
async def on_message(message):
    ...
    for word in words:
        ...
        if match:
            ...
            await message.reply(
                f"Added Spotify track with ID {track_id} to the playlist!",
                delete_after=120,
            )
...
async def status(ctx):
    ...
    if user_id in users:
        await ctx.message.reply(
            f"Listening status: {'listening' if users[user_id]['flag'] else 'not listening'}.",
            delete_after=120,
        )
    else:
        await ctx.message.reply(
...
async def toggle_listen(ctx):
    ...
    if user_id in users:
        ...
        users[user_id]["flag"] = not users[user_id][
            "flag"
        ]  # Toggle the flag (1 to 0 or 0 to 1)
        # print(users[user_id]["flag"],end="\n\n")
        await ctx.message.reply(
            f"Listening flag toggled. Now {'listening' if users[user_id]['flag'] else 'not listening'}.",
            delete_after=120,
        )
    else:
        await ctx.message.reply(
...
async def spotify_login(ctx):
    ...
    try:
        ...
        await ctx.author.send(
            f"Click [here]({auth_url}) to log in to Spotify.", delete_after=120
        )
        ...
        try:
            ...
            if access_token:
                ...
                users[ctx.author.id] = {
                    "username": encrypt_data(access_token),
                    "flag": True,
                }
                GLOBAL_COUNT += 1
                ...
                await ctx.author.send(
                    "Please delete the previous messages for security reasons.",
                    delete_after=120,
                )
            else:
